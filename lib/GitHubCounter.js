...
class GitHubCounter extends BaseCounter {
    ...
    shouldAddCommit(commit) {
         ...
         */
        if (this.usersCommits.indexOf(commit.sha) !== -1) {
            return false;
        }
     ...
     * so loop until there is no next page
     */
    async getUsersRepoCommits(endpoint, params = {}) {
        ...
        commits.forEach(function(commit) {
            if (self.shouldAddCommit(commit)) {
                self.usersCommits.push(commit.sha);
            }
        });
        ...
        if (commits.length === this.itemsPerPage) {
            params.page++;
            await this.getUsersRepoCommits(endpoint, params);
        }
    }

    async getUsersBranchCommits(endpoint, branch) {
        const params = {
            sha: branch,
            per_page: this.itemsPerPage,
            page: 1,
    ...
    async getUsersRepoCommitsCountAll(repo) {
        ...
        for (let i = 0; i < branches.length; i++) {
            await this.getUsersBranchCommits(endpoint, branches[i].name);
        }

        return this.usersCommits.length;
    }
}
