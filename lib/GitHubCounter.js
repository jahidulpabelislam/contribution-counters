...
class GitHubCounter extends BaseCounter {
    ...
    shouldAddCommit(commit, usersCommits, filteredByDatesInAPI) {
        ...
        if (!this.filteredByAuthorInAPI && !this.isCommitter(commit)) {
            return false;
        }

        if (!filteredByDatesInAPI && (this.fromDateObj || this.untilDateObj)) {
    ...
    checkCommits(commits, usersCommits, filteredByDatesInAPI) {
        const self = this;
        commits.forEach(function(commit) {
            if (self.shouldAddCommit(commit, usersCommits, filteredByDatesInAPI)) {
    ...
    async getUsersRepoCommits(endpoint, params = {}, usersCommits = []) {
        ...
        this.checkCommits(commits, usersCommits, true);
    ...
    async getCommitsFromCompare(endpoint, params = {}, usersCommits = []) {
        ...
        this.checkCommits(commits, usersCommits, false);

        // If this response has the max items try next page
        if (count === 1000) {
            // If the last commit is past the Until date bail
            if (this.untilDateObj) {
                const committed = new Date(commits[count - 1].commit.author.date);
                if (committed > this.untilDateObj) {
                    return;
                }
            }
    ...
    async getUsersRepoCommitsCountAll(repo) {
        ...
        // First get commit from default branch
