...
class BitbucketCounter extends BaseCounter {
     ...
     */
    async getUsersPullRequestsCount(endpoint, params) {
        ...
        for (const pullRequestKey in pullRequests) {
            if (!{}.hasOwnProperty.call(pullRequests, pullRequestKey)) {
                continue;
            }
            ...
            if (
                pullRequest.author && (
                    pullRequest.author.uuid == userUUID ||
                    pullRequest.author.nickname.includes(this.username) ||
                    pullRequest.author.display_name.includes(this.username) ||
                    BaseCounter.hasArrayValue(pullRequest.author.nickname, this.userEmailAddresses) ||
                    BaseCounter.hasArrayValue(pullRequest.author.display_name, this.userEmailAddresses) ||
                    BaseCounter.hasArrayValue(pullRequest.author.nickname, this.userNames) ||
                    BaseCounter.hasArrayValue(pullRequest.author.display_name, this.userNames)
                )
            ) {
                count++;
            }
        ...
        if (pullRequestsData.next) {
            params.page++;
            count += await this.getUsersPullRequestsCount(endpoint, params);
        }
    ...
    /**
     * Wrapper function around getUsersPullRequestsCount to trigger the first recursive pull requests GET call
     */
    async getUsersPullRequestsCountAll() {
        const params = {
            state: "all",
            pagelen: 50,
            page: 1,
        ...
        const userUUID = await this.getUserUUID();
        return this.getUsersPullRequestsCount(
            `https://api.bitbucket.org/2.0/pullrequests/${userUUID}`,
            params
        );
    }
