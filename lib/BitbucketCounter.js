...
class BitbucketCounter extends BaseCounter {
     ...
     */
    async getUsersRepoPullRequestCount(endpoint, params) {
        ...
        if (pullRequestsData.next) {
            params.page++;
            count += await this.getUsersRepoPullRequestCount(endpoint, params);
        }
    ...
    /**
     * Wrapper function around getUsersRepoPullRequestCount to trigger the first recursive repo commits GET call
     */
    async getUsersRepoPullRequestsCountAll(repo) {
        const params = {
            state: "all",
            // pagelen: this.itemsPerPage, // todo
            page: 1,
        };
        ...
        return this.getUsersRepoPullRequestCount(repo.links.pullrequests.href, params);
    }
