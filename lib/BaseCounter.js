...
class BaseCounter {
    ...
    async get() {
        ...
        if (this.includePullRequests) {
            // Some counters can get the count by user... other by repo
            usersPullRequestsCount = await this.getUsersPullRequestsCountAll();
        }
        ...
        if (this.includePullRequests) {
            result.pullRequests = usersPullRequestsCount;
        }
