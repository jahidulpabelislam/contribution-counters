...
class BaseCounter {
    ...
    isRepoWithinDates(repo) {
        if (this.fromDateObj) {
            const repoUpdatedDate = new Date(repo[this.repoUpdatedDateField]);
            return repoUpdatedDate >= this.fromDateObj;
        }
    ...
    async getRepos(endpoint, params = {}) {
        ...
        if (count === this.itemsPerPage) {
            if (this.fromDateObj) {
                 ...
                 */
                if (!this.isRepoWithinDates(repos[count - 1])) {
                    return repos;
                }
            }
    ...
    async get() {
        ...
        for (let i = 0; i < repos.length; i++) {
            const repo = repos[i];
            if (!this.isRepoWithinDates(repo)) {
                continue;
            }
