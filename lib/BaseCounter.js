...
class BaseCounter {
    ...
    constructor(config = {}) {
        ...
        if (fromDate !== null && fromDate.trim() !== "") {
            this.fromDateObj = new Date(fromDate);
            this.fromDate = this.fromDateObj.toISOString();
        }

        const { untilDate } = this;
        this.untilDate = null;
        this.untilDateObj = null;
        if (untilDate !== null && untilDate.trim() !== "") {
            this.untilDateObj = new Date(untilDate);
            this.untilDate = this.untilDateObj.toISOString();
        }
    }
    ...
    async getRepos(endpoint, params = {}) {
        ...
        if (this.fromDateObj) {
            ...
            if (repoUpdatedDate < this.fromDateObj) {
                return repos;
            }
        }
