...
class BaseCounter {
    ...
    async get() {
        ...
        if (this.format === "commits" || this.format === "projects" || this.format === "projects_with_commits") {
            result = [];
        }
        ...
        for (let i = 0; i < repos.length; i++) {
            ...
            if (repoCommitsCount > 0) {
                if (repoCommitsCount >= this.minCommits) {
                    if (this.format === "projects" || this.format === "projects_with_commits") {
                        const formattedRepo = this.getFormattedRepoData(repo);
                        if (this.format === "projects_with_commits") {
                            formattedRepo.commits = usersRepoCommits;

                        }

                        result.push(formattedRepo);
                    }
        ...
        if (["commits", "projects", "projects_with_commits", "count_per_project"].indexOf(this.format) === -1) {
