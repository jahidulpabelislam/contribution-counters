...
class BaseCounter {
    ...
    async get() {
        ...
        for (let i = 0; i < repos.length; i++) {
            ...
            const usersRepoCommits = await this.getUsersRepoCommitsAll(repo);
            const repoCommitsCount = Object.keys(usersRepoCommits).length;

            if (repoCommitsCount > 0) {
                if (repoCommitsCount >= this.minCommits) {
                    if (this.format === "commits") {
                        // NOP
                    }
                    else if (this.format === "all" || this.format === "projects" ) {
                        if (this.format === "all") {
                            repo.commits = usersRepoCommits;
                        }

                        result[repo[this.repoIdField]] = repo;
                    }
                    else {
                ...
                if (this.format === "all" || this.format === "projects" ) {
                    ...
                    result = {...result, ...usersRepoCommits};
                }
                else {
                    result.total_commits += repoCommitsCount;
                    result.commits += repoCommitsCount;
                }
            }
        }
