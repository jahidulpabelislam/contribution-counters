...
class GitHub extends API {
    ...
    async getRepoCommits(endpoint, params = {}, usersCommits = []) {
        ...
        if (commits.length === this.itemsPerPage) {
            params.page++;
            await this.getRepoCommits(endpoint, params, usersCommits);
        }
    }
    ...
    async getAllBranchCommits(branch, commitsEndpoint, usersCommits) {
        ...
        await this.getRepoCommits(commitsEndpoint, commitsParams, usersCommits);
    }
    ...
    async getAllRepoCommits(repo) {
        ...
        for (let i = 0; i < branches.length; i++) {
            await this.getAllBranchCommits(branches[i].name, commitsEndpoint, usersCommits);
        }
