...
class Bitbucket extends API {
     ...
     */
    async getRepoCommits(endpoint, params, minFromDate) {
        ...
        for (let i = 0; i < totalCommits; i++) {
            ...
            /**
             * If this commit was before the minFromDate user requested bail very early and return.
             * As any further commits will be before this date.
             * This kind of filtering is done because Bitbucket doesn't allowing filtering by date on commits so we filter on own end.
             */
            const commitFromDate = new Date(commit.date);
            if (commitFromDate < minFromDate) {
        ...
        if (commitsData.next) {
            params.page++;
            totalUsersCommits += await this.getRepoCommits(endpoint, params, minFromDate);
        }
    ...
    async getAllRepoCommits(repo) {
        ...
        return await this.getRepoCommits(repo.links.commits.href, params, fromDateObj);
    }
